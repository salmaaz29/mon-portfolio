...
function Chatbot({ open, onClose }) {
  const [msgs, setMsgs] = useState([{ role:"assistant", text:"👋 Bonjour ! Je suis l'assistant IA de Salma. Posez-moi une question sur elle !" }]);
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: apiMsgs, system: SALMA_CONTEXT })
      });
      const rawText = await res.text();
      if(!rawText || rawText.trim()===""){
        throw new Error("Réponse vide du serveur. Vérifie que ANTHROPIC_API_KEY est bien définie dans Vercel > Settings > Environment Variables.");
      }
      let data;
      try { data = JSON.parse(rawText); } catch(e){ throw new Error("Réponse invalide du serveur : "+rawText.slice(0,120)); }
      if(!res.ok){
        throw new Error(data.error || "Erreur serveur "+res.status);
      }
      const reply = data.content?.[0]?.text || "Désolée, une erreur est survenue.";
  ...
  if(!open) return null;
  return (
    <div style={{position:"fixed",bottom:24,right:24,width:380,maxWidth:"calc(100vw - 48px)",height:520,background:"linear-gradient(145deg,#0a0e1a 0%,#111827 100%)",borderRadius:22,border:"1px solid rgba(233,69,96,.25)",boxShadow:"0 25px 60px rgba(0,0,0,.6), 0 0 80px rgba(233,69,96,.06)",display:"flex",flexDirection:"column",zIndex:1000,overflow:"hidden",animation:"chatSlide .35s cubic-bezier(.34,1.56,.64,1)"}}>
      {/* Header */}
      <div style={{padding:"14px 18px",background:"linear-gradient(135deg,rgba(233,69,96,.15),rgba(233,69,96,.05))",borderBottom:"1px solid rgba(233,69,96,.2)",display:"flex",justifyContent:"space-between",alignItems:"center",backdropFilter:"blur(6px)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:34,height:34,borderRadius:"50%",background:"linear-gradient(135deg,#e94560,#c23152)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,boxShadow:"0 0 14px rgba(233,69,96,.4)"}}>🤖</div>
          <div>
            <div style={{color:"#fff",fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:600,letterSpacing:.3}}>Salma AI</div>
            <div style={{color:"#6ee7b7",fontSize:10.5,display:"flex",alignItems:"center",gap:5,marginTop:1}}><span style={{width:6,height:6,borderRadius:"50%",background:"#4ade80",display:"inline-block",boxShadow:"0 0 6px #4ade80"}}/> En ligne</div>
          </div>
        </div>
        <button onClick={onClose} style={{background:"none",border:"none",color:"#6b7280",cursor:"pointer",fontSize:18,lineHeight:1,transition:"color .2s"}} onMouseEnter={e=>e.target.style.color="#e94560"} onMouseLeave={e=>e.target.style.color="#6b7280"}>✕</button>
      </div>
      {/* Messages */}
      <div style={{flex:1,overflowY:"auto",padding:"16px 14px",display:"flex",flexDirection:"column",gap:10}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            <div style={{maxWidth:"82%",padding:"9px 13px",borderRadius:m.role==="user"?"16px 16px 3px 16px":"16px 16px 16px 3px",background:m.role==="user"?"linear-gradient(135deg,#e94560,#c23152)":"rgba(255,255,255,.05)",color:"#e5e7eb",fontSize:12.5,lineHeight:1.55,fontFamily:"'Inter',sans-serif",border:m.role==="user"?"none":"1px solid rgba(255,255,255,.08)",boxShadow:"0 2px 6px rgba(0,0,0,.25)"}}>{m.text}</div>
          </div>
        ))}
        {loading&&<div style={{display:"flex",justifyContent:"flex-start"}}><div style={{padding:"9px 14px",borderRadius:"16px 16px 16px 3px",background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.08)"}}><span style={{color:"#6b7280",fontSize:18,animation:"chatPulse 1.1s infinite"}}>● ● ●</span></div></div>}
        <div ref={botRef}/>
      </div>
      {/* Input */}
      <div style={{padding:"10px 14px",borderTop:"1px solid rgba(255,255,255,.07)",display:"flex",gap:8,background:"rgba(0,0,0,.2)"}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Posez une question..." style={{flex:1,background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:14,padding:"9px 13px",color:"#e5e7eb",fontSize:12.5,fontFamily:"'Inter',sans-serif",outline:"none",transition:"border .2s"}} onFocus={e=>e.target.style.borderColor="rgba(233,69,96,.5)"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,.1)"}/>
        <button onClick={send} disabled={loading} style={{background:"linear-gradient(135deg,#e94560,#c23152)",border:"none",borderRadius:14,width:40,height:40,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,color:"#fff",opacity:loading?.5:1,transition:"transform .2s,opacity .2s",boxShadow:"0 3px 12px rgba(233,69,96,.35)"}} onMouseEnter={e=>!loading&&(e.target.style.transform="scale(1.08)")} onMouseLeave={e=>e.target.style.transform="scale(1)"}>↗</button>
      </div>
    </div>
  );
}
